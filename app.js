...
(function () {
  ...
  function setupPaymentListListeners(containerEl, onSelectionChange) {
    ...
    containerEl.addEventListener('keydown', event => {
      if (event.target.matches('.payment-card-header') && (event.key === 'Enter' || event.key === ' ')) {
  ...
  function renderFeaturedCard(cardData, visibleServices) {
    ...
    // Offer Quantity
    if ($('heroQuantity')) {
      const qVal = String(targetPlan.label).trim();
    ...
    // Offer Price
    if ($('heroPrice')) {
      const pVal = String(money(targetPlan.price) + ' جنيه').trim();
  ...
  function renderGrid() {
    ...
    grid.innerHTML = services.map(s => {
      ...
      const entryPlan = (s.plans || []).find(p => p.available && p.price === minPrice);
  ...
  function applyFilters() {
    ...
    if ($('discoverySearch')) $('discoverySearch').value = query;
  ...
  function openService(idOrService, planId) {
    ...
    $('serviceDialog').querySelector('.checkout-scroll-body')?.scrollTo({ top: 0, behavior: 'instant' });
  }

  $('serviceDialog')?.addEventListener('click', event => {
    const jump = event.target.closest('[data-checkout-jump]');
    if (!jump) return;
    const target = $(jump.dataset.checkoutJump);
    if (!target || !$('serviceDialog').contains(target)) return;
    target.setAttribute('tabindex', '-1');
    const scroller = $('serviceDialog').querySelector('.checkout-scroll-body');
    scroller.scrollTo({ top: scroller.scrollTop + target.getBoundingClientRect().top - scroller.getBoundingClientRect().top - 8, behavior: 'instant' });
  ...
  const cartInertNodes = new Map();
  function openCart() {
    if ($('cartDrawer')?.hidden) {
      cartOpener = document.activeElement;
      previousOverflow = document.body.style.overflow;
      document.body.style.overflow = 'hidden';
      document.querySelectorAll('body > header, body > main, body > footer, body > nav, body > section, .announcement, .floating-contact').forEach(el => {
        cartInertNodes.set(el, el.inert); el.inert = true;
      });
    }
  ...
  document.addEventListener('keydown', event => {
    if (!$('cartDrawer') || $('cartDrawer').hidden || document.querySelector('dialog[open]')) return;
    if (event.key === 'Escape') { event.preventDefault(); closeCart(); }
    if (event.key !== 'Tab') return;
    const nodes = [...$('cartDrawer').querySelectorAll('button:not(:disabled), a[href], input:not(:disabled), select:not(:disabled), textarea:not(:disabled), [tabindex="0"]')].filter(el => el.getClientRects().length);
    const first = nodes[0], last = nodes[nodes.length - 1];
    if (!$('cartDrawer').contains(document.activeElement)) { event.preventDefault(); first?.focus(); }
  ...
  $('cartItemsList')?.addEventListener('click', event => {
    ...
    const next = $('cartItemsList').querySelector(`[data-cart-action="${action}"][data-cart-index="${idx}"]`) || $('closeCartBtn');
  ...
  async function renderAdminOrders() {
    ...
    try {
      ...
      if (window.KenoFirebase && typeof window.KenoFirebase.computeKPIs === 'function') {
        ...
        if ($('kpiRevenue')) $('kpiRevenue').textContent = Number(kpi.totalRevenue || 0).toLocaleString('en-US') + ' ج.م';
  ...
  $('publishButton')?.addEventListener('click', async () => {
    ...
    try {
        ...
        if (idToken) {
          ...
          const publishRes = await fetch('/api/publish', {
            ...
            body: JSON.stringify({
              ...
              baseUpdatedAt: baseData?.updatedAt || null,
      ...
      if (!firestoreOK && !githubOK) {
        throw new Error('تعذر النشر على السحابة. المسودة محفوظة ولم تُنشر؛ راجع الاتصال وإعدادات النشر ثم حاول مرة أخرى.');
      }
      // --- Update local state only after a confirmed remote write ---
      ...
      if (statusParts.length) {
        showAdminMessage('تم النشر على: ' + statusParts.join(' + ') + '. ' + (firestoreOK ? 'تم تحديث الكتالوج السحابي. ' : 'ظهورها للزوار يعتمد على اكتمال نشر الاستضافة. ') + ((!firestoreOK || !githubOK) ? 'تعذر مسار النشر الآخر؛ صدّر نسخة احتياطية وراجع إعداداته.' : ''));
  ...
  document.addEventListener('keydown', event => {
    if (event.key === 'Enter' || event.key === ' ') {
      ...
      if (card && !card.matches('button, a') && (event.target === card || !event.target.closest('button, a, input, select, textarea'))) {
        event.preventDefault();
